...
| Planet and moon topology | Direct-preview | `orbitAround`, `orbitAroundPlanet`, `orbitIndex`, `number` | After galaxy creation; low | Primary `number` values identify star-orbiting parents. Moon `number` values are local display ordinals and may repeat primary or other moon values, so they are not system-wide identity. `orbitAround` selects the primary parent number; an available `orbitAroundPlanet` may corroborate that relationship. Moon and satellite counts are deterministic aggregations over these scoped fields. |
