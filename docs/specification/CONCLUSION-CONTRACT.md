...
| `FS-POWER` | Which sibling moons provide renewable-power opportunities? | Preview rotation, wind, solar, and verified parent facts | One or two siblings of the home moon receive independent Solar/Wind verdicts at 0.4/1.15; tidal-lock guidance is unchanged. | No home-planet or direct-star-home power advice. Never predict realized build output. |
| `FS-GAS-ROUTE` | Which early-to-midgame giant-product opportunities exist? | Preview gas products and rates | Present one starter-giant verdict: Fire Ice supports, Deuterium does not support. Also show the nearest individual giant with Deuterium >= 0.15/s using the 2.5/10 ly distance columns. | No duplicate absence or Hydrogen verdict for the starter giant; never imply mineable deposits. The statistics counterpart alone retains the inclusive 8.125 ly cap and is titled Nearest Rich Deuterium Gas-Giant. |
| `MF-RESOURCE-SYSTEM.rare` | Where are the targeted factory rares? | Complete per-system finite deposits | Nearest qualifying system for Spiniform > 900,000, Optical Grating > 600,000 and Organic presence; distance columns use 2.5/10 ly. | Deposits can span planets and need no Aquatica theme. Shared-system descriptions merge. Other rare summaries and Many rares are omitted; statistics candidate selection is unchanged. |
| `MF-RESOURCE-SYSTEM.plentiful` | Where is substantial mixed supply? | Complete per-system finite totals | At least three resource types each strictly > 40 million; present the nearest three eligible systems with 2.5/10 ly distance columns. | Sum across planets, preserve resource names and exclude rate resources such as oil. No global score. |
| `FS-RESOURCES` | Are starter deposits robustly strong, limited, or preference-sensitive for early progression? | Complete raw birth-system veins and oil | Per-resource, setting-specific amount and distribution intervals determine the stable outcome. | **Unknown** until ranges and complete coverage exist. Never convert the result into starter viability. |
...
| `MF-SPHERE-GEOMETRY` | Which systems provide shell-radius or orbital-containment opportunities? | Preview and derived sphere geometry | Present nearest-home candidates with luminosity >= 2.0; existing shell/count ranges are unchanged. | Full orbit containment includes the parent radius for moons and uses the native maximum sphere radius in meters. Containment appears only in Sphere / energy. Never infer receiver effectiveness. |
