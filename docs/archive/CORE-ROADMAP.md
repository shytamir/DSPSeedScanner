...
The core exposed presentation-neutral reports for the separately planned New
Game panel. This roadmap did not build that panel or a substitute interface.

## Governing constraints

Every story inherited the accepted project definition and specification. In
particular, the roadmap required that:
...
- preview and raw work each operated on one requested identity while the safe
  runtime boundaries were being established;
...
Implementation used the smallest direct design that preserved the accepted
architecture. New abstractions, compatibility layers, persistence, and
extension systems required demonstrated scope within an active story.
...
## Phase 3 - Establish and use the raw-generation boundary
...
deposits rather than preview proxies.

**Return:** A cancellable developer-invoked birth-system operation returns every
...
- a new raw-evidence report that does not mutate the prior preview report.
...
**Produced:** The first exact raw-evidence player-decision report boundary and a
