...
On completion, each successfully loaded New Game cluster preview initiated one
resolution attempt for its complete generation identity. A valid local cache
hit resolved immediately; otherwise the panel showed immediate preview
conclusions, ran one bounded full scan automatically, displayed visible
activity while work remained, and replaced that activity with concise neutral
conclusions when complete.

Changing the seed or leaving the preview made obsolete work ineligible to
update the panel. The player did not need to invoke, configure, or interpret
...
Stories PRES-01 through PRES-06 used focused automated tests, deterministic
harnesses, builds, and automated game-linked probes where runtime behavior had
to be exercised. They did not require human in-game validation.

Human in-game validation occurred only in PRES-07. This accepted sequencing
risk did not relax the final runtime, responsiveness, lifecycle, or
presentation acceptance gate. Failures discovered there were repaired within
PRES-07 only when necessary to satisfy an already stated requirement; they did
...
- birth-topology conclusions identified the system without naming the decisive
  arrangement;
- compact-expansion conclusions showed distances but omitted the system roles
  whose grouping was classified;
- sphere conclusions omitted the contained-orbit count, and rare-resource
  access omitted the destination system when the available subject attribution
  did not identify it;
- Dark Fog opportunity text could be identical across different outcome
  classes, birth exposure remained terse, and broader real-seed sampling was
  needed before changing or removing the context;
- decision-relevant traits exposed mechanically formatted identifiers; and
- three examples plus `+N` favored breadth over explaining why the examples
  received their class. The native scrollbar was also visually heavier than
  the rest of the panel, but was cosmetic.

The subsequent roadmap was also required to contain a bounded scan-cadence
story. It could accept a longer cache-miss duration in exchange for yielding
more frequently and reducing visible frame disruption. It had to preserve
main-thread generation, serialization, cancellation, progress, restoration of
runtime statics, and the one-attempt lifecycle; the handoff did not authorize
parallel or background DSP generation.
...
This roadmap did not add adaptive panel placement, player scoring or required
