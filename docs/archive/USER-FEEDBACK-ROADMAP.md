...
The accepted conclusion panel remained the configured surface for bounded seed
conclusions. This roadmap did not replace its Strength, Preference-sensitive,
and Limitation lanes. FEED-01 corrected that panel only within its explicit
scope; the factual stories did not alter it.

A separate statistics panel presented neutral facts under `Home system` and
`Cluster`. Its corner was the horizontal opposite of the configured conclusion
corner: bottom right paired with bottom left, and top right paired with top
left, in either direction. Its bounds were derived by horizontally reflecting
the resolved conclusion-panel bounds while preserving the exact vertical
position, width, and height. The horizontal UI anchor and pivot were mirrored
while the top-or-bottom anchor was retained. The opposite corner's independent
clearance rules were not applied, and no second placement setting was added.

The statistics panel received its own presentation document, renderer,
scrolling, and visual state. It consumed the same active preview session,
normalized evidence, and shared layout result as the conclusion panel, but
neither panel owned, queried, or drove the other. Shared semantic values and
geometry belonged at the runtime or presentation boundary rather than in
either renderer.

Both panels retired from the same preview lifecycle authority. Preview facts
could appear immediately; raw-scan facts appeared only after completion or a
valid cache hit. Unavailable facts were omitted. Measurements used concise
natural labels and explicit game units. Cached statistics retained only their
bounded presentation payload, never raw planets or a complete cluster graph.

The configured conclusion corner remained authoritative for the pair. An
independent statistics-panel position, a generic multi-panel framework, and
changes to the accepted conclusion-panel dimensions or anchor were not part of
this roadmap.
...
- FEED-04 followed FEED-03 and supplied one authoritative title value to both
  panels without coupling their renderers.
- FEED-05 first projected the home-system inventory; FEED-06 then joined raw ore
  evidence to those body rows.
- FEED-07, FEED-08, and FEED-09 independently reused FEED-03's cluster
  primitives for their own bounded selections. FEED-07 excluded Unipolar
  Magnets; FEED-08 owned them exclusively.
- FEED-10 was the first consumer of FEED-03's titled cluster subsections and
  remained in the statistics panel's single scroll flow.

Acceptance of one story did not implicitly accept another.
